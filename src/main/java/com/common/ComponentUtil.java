...
public class ComponentUtil
{
    ...
    public String print(String entity_code, String column_code,
			String default_value,String display_state)
    {
    	return print(entity_code, column_code, default_value,null,null,null,display_state, null,false);
    }
	public String print(String entity_code, String column_code,
			String default_value, boolean isPkmodify)
	{
		return print(entity_code, column_code, default_value, "", isPkmodify);
	}

	public String print(String entity_code, String column_code,
			String default_value, String parameter, boolean isPkmodify)
	{
		return print(entity_code, column_code, default_value, null, null, null,null,
				parameter, isPkmodify);
	}

	public String print(String entity_code, String column_code,
			String default_value, String display_width, String display_height,
			String display_style, String display_state,String parameter, boolean isPkmodify)
	{
		...
		{ new Parameter.String(entity_code.toUpperCase()),
		...
		try
		{
			...
			String column_name = dt.get(0).getString("column_name")
					.toLowerCase();
			String is_primarykey = dt.get(0).getString("is_primarykey")
					.toLowerCase();
			String foreign_entity = Format.NullToBlank(
					dt.get(0).getString("foreign_entity")).toLowerCase();
			String column_type = dt.get(0).getString("column_type")
					.toLowerCase();
			String column_length = dt.get(0).getString("column_length")
					.toLowerCase();
			String code_entity = Format.NullToBlank(
					dt.get(0).getString("code_entity"));
			String edit_code = Format.NullToBlank(
					dt.get(0).getString("edit_code")).toLowerCase();
			// 画组件
			if (edit_code.equals("") || edit_code.equals("00010001"))
			{
				...
				sBuilder.append("<input class=\"easyui-textbox\" type=\"text\" id=\""+item_name+"\"   name=\""+item_name+"\" "+display_state+" data-options=\"required:true\" value=\""+default_value+"\" />");
				/*sBuilder.append("<input type=\"text\" "+display_state+" name=\"" + item_name
						+ "\" id=\"" + item_name + "\" class=\""
						+ display_style + "\" style=\"width:" + display_width
			...
			if (edit_code.equals("00010002"))
			{
				...
				if(default_value.equals("")){
					sBuilder.append("<input class=\"easyui-textbox\" type=\"text\"  id=\""+item_name+"\"    name=\""+item_name+"\" "+display_state+" data-options=\"required:true\"  onkeyup=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\" " +
									"onafterpaste=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\" />");
					
				/*	sBuilder.append("<input type=\"text\""+display_state+" name=\"" + item_name
							+ "\" id=\"" + item_name + "\" class=\""
							+ display_style + "\" style=\"width:" + display_width
							+ ";height=" + display_height + "\" onkeyup=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\" " +
									"onafterpaste=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\"  >");*/
					
				}else {
					sBuilder.append("<input class=\"easyui-textbox\" type=\"text\"   id=\""+item_name+"\"   name=\""+item_name+"\" "+display_state+" data-options=\"required:true\"  onkeyup=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\" " +
							"onafterpaste=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\" value="+Integer.parseInt(default_value)+" />");
		
					/*sBuilder.append("<input type=\"text\" name=\"" + item_name
							+ "\" id=\"" + item_name + "\" class=\""
							+ display_style + "\" style=\"width:" + display_width
							+ ";height=" + display_height + "\" onkeyup=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\" " +
									"onafterpaste=\"if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\\D/g,'')}\"  value="+Integer.parseInt(default_value)+">");*/
		...
		/*		sBuilder.append("<select  name=\"" + item_name
						+ "\" id=\"" + item_name + "\" class=\""
						+ display_style + "\" style=\"width:" + display_width
			...
			if (edit_code.equals("00010005"))
			{
				...
				//sBuilder.append("<input type='radio' name='"+item_name+"' id='"+item_name+"' value='"++"'");
			...
			if (edit_code.equals("00010009"))
			{
				...
				sBuilder.append("<textarea name=\"" + item_name
						+ "\" id=\"" + item_name + "\" class=\""
						+ display_style + "\" style=\"width:" + display_width
						+ ";height:"+display_height+" \" value=\""
			...
			if (edit_code.equals("00010010"))
			{
			   //文件上传
				...
				if (display_height == null)
					display_height = this.Height_normal;
				sBuilder.append("<input type=\"file\" name=\""+item_name
						+"\" id=\""+item_name+"\"+class=\""+display_style
			...
			{
				// 长度检查
				CheckScript = CheckScript + "if (document.all('" + item_name
				+ "').value.length>" + column_length
				+ ") check_res=check_res+'数据项：" + column_name
				+ "长度超过系统允许的值（" + column_length + "）\\n';\n";
				 //数字的校验
				//CheckScript = CheckScript + "if ('"+column_type+"'=='number')" +
				//		"if(!IsNumber(document.all('"+item_name+"').value))" +
				//		"check_res=check_res+'数据项：" + column_name
				//		+ "必须为数字型\\n';\n";
				//日期型数据的校验
				CheckScript = CheckScript + "if ('"+column_type+"'=='date')" +
				"if(!isDate(document.all('"+item_name+"').value))" +
				"check_res=check_res+'数据项：" + column_name
				+ "必须为日期型\\n';\n";
			}
			...
			if(is_primarykey.equals("y")){
				CheckScript = CheckScript + "if (document.all('" + item_name
				+ "').value==null||document.all('" + item_name
				+ "').value=='') check_res=check_res+'数据项：" + column_name
				+ "为表单主键不能为空\\n';\n";
			}
			// 其它的校验
