...
public static String getselectItem(String code_entity,String old_value ){
		...
		if(old_value.equals("")){
			...
			res+=
				   "<option value="+"\'"+old_value+"\'"+">"+old_value+"</option>";
			res+=
				   "<option value="+"\'\'"+">--请选择--</option>";
		}
...
public String  choose(String group_code) throws Exception{
	...
	if (dt_group!=null && dt_group.getRowsCount()>0)
	{
		for (int j=0;j<dt_group.getRowsCount();j++)
		{
			...
			{
				sBuilder.append("<option value='querymanage.jsp?group_code="+r_group.getString("group_code")+"' selected>"+Coder.getCdfromccm(r_group.getString("group_code"))+r_group.getString("group_name")+"</option>");
			...
			{
				sBuilder.append("<option value='querymanage.jsp?group_code="+r_group.getString("group_code")+"'>"+Coder.getCdfromccm(r_group.getString("group_code"))+r_group.getString("group_name")+"</option>");
		 	...
		 	for (int i=fnn;i<dt.getRowsCount();i++)
			{
				...
				try {
						...
						if(default_value.equals("")){
							...
							res.append("<input type='radio' name='"+item_name+"' id='"+item_name+"' value='"+r.getString("code_id")+"'/><span style='font-size:12px'>"+r.getString("code_name")+"</span>");
						}else{
							if(default_value.equals(r.getString("code_id")))
							{
								res.append("<input type='radio' name='"+item_name+"' id='"+item_name+"' value='"+r.getString("code_id")+"' checked/><span style='font-size:12px'>"+r.getString("code_name")+"</span>");
							}else {
								res.append("<input type='radio' name='"+item_name+"' id='"+item_name+"' value='"+r.getString("code_id")+"'/><span style='font-size:12px'>"+r.getString("code_name")+"</span>");
							}
    	 ...
    	 if(dt!=null&&dt.getRowsCount()>0){
 	    	...
 	    	for(int i=fnn;i<dt.getRowsCount();i++){
 	    		...
 	    		try {
 	    			...
 	    			res.append("<input type='checkbox' name='"+item_name+"' id='"+item_name+"' value='"+r.getString("code_id")+"'/>"+r.getString("code_name"));
        	...
        	if(dt!=null&&dt.getRowsCount()>0){
     	    	...
     	    	for(int i=fnn;i<dt.getRowsCount();i++){
     	    		...
     	    		try {
     	    			...
     	    			res.append("<input type='checkbox' name='"+item_name+"' id='"+item_name+"' value='"+r.getString("code_id")+"' checked/>"+r.getString("code_name"));
     	    			}
     	    			else{
     	    				res.append("<input type='checkbox' name='"+item_name+"' id='"+item_name+"' value='"+r.getString("code_id")+"' />"+r.getString("code_name"));
     	    			}
