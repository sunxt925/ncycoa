...
public void deployStdFlow(){
	ProcessEngine pe = Configuration.getProcessEngine();
	IdentityService identityService=pe.getIdentityService();
	RepositoryService rs = pe.getRepositoryService();
	ZipInputStream zis = new ZipInputStream(this.getClass().getResourceAsStream("/com/workflow/std/stdflow.zip"));
	rs.createDeployment().addResourcesFromZipInputStream(zis).deploy();
}
public boolean startProcessinstance(String staffcode) throws Exception{
	...
	DataTable yingxiaoDT=getMemberList(shenhe,orgyingxiao);
	...
	DataTable zhuanmaiDT=getMemberList(shenhe,orgzhuanmai);
	...
	DataTable anquanDT=getMemberList(shenhe,organquan);
	...
	DataTable wuliuDT=getMemberList(shenhe,orgwuliu);
	...
	DataTable jichuDT=getMemberList(shenhe,orgjichu);
	...
	map.put("WeiYuanHuiGroup",weiyuangroup);
	//////////////////////////////////////////////////////////////////////////////
		String towhere="";
	...
	RepositoryService rs = pe.getRepositoryService();
	String pdId="";
	List<ProcessDefinition> lists = rs.createProcessDefinitionQuery().list();
	for (ProcessDefinition pd : lists) {
		if(pd.getName().equals("stdflow")){
			pdId=pd.getId();
		}
	}
	ProcessInstance pi=es.startProcessInstanceById(pdId,map);
...
public DataTable isApplyMan(String staffcode) {
	try {
		...
		System.out.println("SQL::::"+urlString);
