...
<%
	...
	Participant participant = new Participant(taskno,participantcode); 
%>
...
<form name="form1" id="form1" method="post" action="../servlet/PageHandler">
  ...
  <tr>
    <td colspan="3" valign="top" class="main_table_centerbg" align="center"><table width="100%" border="1" cellspacing="0" cellpadding="0" class="table_list1">
      <tr>
        <td width="20%"><div align="right">参与人任务说明：</div></td>
        <td width="80%">
        
        <%out.print(cu.print("MONTHTASKPARTICIPANT","TASKDESC",participant.getTaskdesc())); %>
        <!-- 
	  ...
	  <tr>
        ...
        <input name="gdls" type="text" class="input1" id="gdls" onKeyDown="EnterKeyDo('F8()')" onKeyUp="this.value=this.value.replace(/\D/g,'')" value="0" size="3" maxlength="3"  onafterpaste="this.value=this.value.replace(/\D/g,'')" style="text-align:right">
         ...
         -->
         <%out.print(cu.print("MONTHTASKPARTICIPANT","MEMO",participant.getMemo())); %>
        </td>
      </tr>
         <input type="hidden" name="entity" id="entity" value="MONTHTASKPARTICIPANT"/>
          <input type="submit" name="Submit" value="提交" style="display:none">
            <input type="reset" name="reset" value="重置" style="display:none">
            <input type="hidden" name="MONTHTASKPARTICIPANT.TASKNO" id="MONTHTASKPARTICIPANT.TASKNO" value=<%=participant.getTaskno() %>>
            <input type="hidden" name="old_TASKNO" id="old_TASKNO" value=<%=participant.getTaskno() %>>
             <input type="hidden" name="MONTHTASKPARTICIPANT.PARTICIPANTCODE" id="MONTHTASKPARTICIPANT.PARTICIPANTCODE" value=<%=participant.getParticipantcode() %>>
            <input type="hidden" name="old_PARTICIPANTCODE" id="old_TPARTICIPANTCODE" value=<%=participant.getParticipantcode() %>>
