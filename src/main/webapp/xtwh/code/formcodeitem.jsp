...
<%
	...
	FormCodeItem formCodeItem=new FormCodeItem();
	
	DataTable dt=formCodeItem.getFormcodeitemlist(prmcod,page_no,per_page);
	DataTable dtcount=formCodeItem.getAllFormcodeitemlist(prmcod);
...
function F3()
{
	...
	var prmcod="<%=prmcod%>";	
	var re=window.showModalDialog("formcodeitemadd.jsp?prmcod="+prmcod+"&sid="+rand,window,"scroll=no;status=no;dialogWidth=500px;dialogHeight=200px;center=yes;help=no;");
	if(re=="refresh")
...
var prmcod="<%=prmcod%>";
var rand=Math.floor(Math.random()*10000);
	var re=window.showModalDialog("formcodeitemmod.jsp?prmcod="+prmcod+"&prmcon="+para+"&sid="+rand,window,"scroll=no;status=no;dialogWidth=500px;dialogHeight=200px;center=yes;help=no;");
	if(re=="refresh")
	{window.location.reload();
	}
...
document.all("act").value="del";
document.getElementById("prmcon").value=para;
if (confirm("确定要删除选中的记录？"))
		{
			document.all("form1").submit();
		...
		<table width="100%" height="100%" border="0" cellpadding="0"
			...
			<form name="form1" id="form1" method="post"
				...
				<tr>
					...
					&nbsp;&nbsp;
					<a href="formcodemanange.jsp">返回</a>
						&nbsp;&nbsp;
					...
					<td colspan="3" valign="top" class="main_table_centerbg"
						...
						<%
							if (dt != null && dt.getRowsCount() >= 0)
							{
								...
								tu.setCheckBoxValue("prmcod,prmcon");
					
								tu.setRowValue("操作","<a href=\"#\" onclick=\"modify('@prmcon@')\" class=\"button4\">修改</a> <a href=\"#\" onclick=\"del('@prmcon@')\" class=\"button4\">删除</a>");
						...
						<input name="prmcod" id="prmcod" type="hidden" value="<%=prmcod %>"/>
						<input name="prmcon" id="prmcon" type="hidden" value=""/>
						<input name="act" type="hidden" id="act" value="del">
						<input name="action_class" type="hidden" id="action_class"
							value="com.action.system.FormCodeItemAction">
					</td>
				</tr>
				<tr>
