...
function validate(value,field,TaskValue) {
	//field.checked="checked";
	var radios=document.form1.radiobutton;
	
	for(var i=0;i<radios.length;i++){
		if(radios[i].value==TaskValue){
...
function selectPerson(){
	...
	var results= showModalDialogWin("systemrole_member_new.jsp",1000,600);
...
function selectDepartment(){//选择的时候，需要判断是否重复，重复的值自动过滤
	var results= showModalDialogWin("unitmanage.jsp",1000,600);
	...
	if (confirm("确定按选定值设置权限？"))
	{
		...
		for(var i=0;i<radios.length;i++){
			if(radios[i].checked==true){
				document.getElementById("worktype").value=radios[i].value;
			}
		}
...
<form name="form1" id="form1" method="post" action="../../servlet/PageHandler">
  ...
  <tr >
	...
	<table width="100%" height="40" border="0" cellpadding="3" cellspacing="5" bgcolor="b2d5ff">
      <tr>
        <td width="100%" bgcolor="#FFFFFF">请选择任务类别:
           <input type="radio" name="radiobutton" value="单位绩效" checked="checked" />单位绩效
           <input type="radio" name="radiobutton" value="部门绩效" />部门绩效
           <input type="radio" name="radiobutton" value="个人绩效" />个人绩效
		</td>
      </tr>
    </table>
	</td>
...
<td colspan="3" valign="top" height="100%">
   <table width="100%" height="100%">
	...
	<th>序列号</th><th>员工编码</th><th>姓名</th><th>任务类别</th>
	<%
	if (dt!= null && dt.getRowsCount()>0)
	{
		...
		{
	%>		
			<tr onMouseOver="this.style.backgroundColor='#D0E9ED';this.style.cursor='hand'" onMouseOut="this.style.backgroundColor=''" onClick="validate(this.children[1].innerText,this,this.children[3].innerText);" >
			...
			<td><%= dt.get(i).getString("tasktype")%></td>
			</tr>
	<%
		}
	}
	...
	<th>序列号</th><th>机构编码</th><th>名称</th>
      ...
      <input name="action_class" type="hidden" id="action_class" value="com.action.system.SysTBM_TaskPersonAction">
      <input name="staffcode" type="hidden" id="staffcode" value="">
      <input name="orgcode" type="hidden" id="orgcode" value="">
      <input name="worktype" type="hidden" id="worktype" value="">
	</td>   
  </tr>
  <tr >
