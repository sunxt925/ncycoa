  ...
  <tr>
    <td colspan="3" valign="top" class="main_table_centerbg" align="center"><table width="100%" border="1" cellpadding="0" cellspacing="0" class="table_list">
      ...
      <tr onMouseOver="this.style.backgroundColor='#D0E9ED'" onMouseOut="this.style.backgroundColor=''">
	    <td align="center"><input type="checkbox" id="items" name="items" value="<%=r.getString("level_code")%>" class="checkbox1"></td>
        <td align="left"><%=Coder.getCdfromccm(r.getString("level_code"))%><%=r.getString("level_code")%></td>
        <td align="left"><%=Coder.getCdfromccm(r.getString("level_code"))%><%=r.getString("menu_name")%></td>
		<td align="left"><%=r.getString("menu_url")%></td>
        <td align="center"><%=Coder.getEnumValue(r.getString("menu_type"),"0,普通菜单;1,查询菜单")%></td>
		<!--<td align="left"><%=r.getString("menu_type")%></td>
        --><td align="center"><a href="module_mod.jsp?level_code=<%=r.getString("level_code")%>" class="button4">修 改</a></td>
      </tr>
	  <%
	  	}}
	  %>
	...
	%>
     ...
     <input name="act" type="hidden" id="act" value="del"><input name="dao_class" type="hidden" id="dao_class" value="com.dao.SystemModule.SystemModuleDao"></td>
  </tr>
  </tr>
  <tr>
