...
<body>
<div>
<img alt="" src="pic.jsp?id=<%=processInstanceId %>" style="text-align: center;position:absolute;left:0.5%;top:0.5%;">
<%for(ActivityImpl actImpl : actImpls){ %>
<div id="flow" style="position:absolute; border:0px solid red;left:<%=actImpl.getX() %>px;top:<%=actImpl.getY()%>px;width:<%=actImpl.getWidth()-10 %>px;height:<%=actImpl.getHeight()-10 %>px;" title="<%=message%>"></div>
<%} %>
</div>
<div>
<%=message%>
