...
<%
	...
	String sql="select * from system_ftp where file_type=2 order by file_id";
...
function F3()
{
	...
	window.open("CreatePPT.jsp?sid="+rand,"_self");
}
function F6()
{
	window.open("OpenTemplate.jsp","_self");
}
function F4()
{
      ...
      <tr onMouseOver="this.style.backgroundColor='#D0E9ED'" onMouseOut="this.style.backgroundColor=''">
        ...
        <td align="center"><%=r.getString("file_name")%></td>
          <td style="text-align:center;">
			<a href="PPTOpen.jsp?f_url=<%=r.getString("file_url")%>&f_name=<%=r.getString("file_name")%>&f_id=<%=r.getString("file_id")%>"><span style=" color:Green;">编辑</span></a>&nbsp;&nbsp;&nbsp;
			<a href="PPTDelete.jsp?f_url=<%=r.getString("file_url")%>&f_name=<%=r.getString("file_name")%>&f_id=<%=r.getString("file_id")%>" ><span style=" color:Green;">删除</span></a>&nbsp;&nbsp;&nbsp;				
		</td> 
      </tr>
	  <%
	  	}}
